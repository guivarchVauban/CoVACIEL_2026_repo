...
class NavTunnelPIDNode(Node):
    def __init__(self):
        super().__init__('car_node_orchestrateur')
        self.get_logger().warning(">>> NAV CAPTAIN V3.6 : OVERTAKE INTELLIGENT <<<")
        ...
        self.direction_evasion = 0.0
        self.evasion_until = 0.0
        self.EVASION_DURATION = 0.45
    ...
    def toggle_donut(self):
        if self._robot_mode != 1:
            self.get_logger().warn("Active le robot d'abord (ESPACE) !")
            return
        ...
        if self.current_state == "AVANCE":
            ...
            if not self.overtake_mode:
                ...
                if now < self.drift_until:
                    cmd.linear.x, limit_angle = 0.67, 1.9; mode_label = "DRIFT"
                else:
...
def main(args=None):
    ...
    try:
        ...
        has_tty = False
        node.get_logger().info("Pas de TTY — contrôle clavier désactivé (mode background)")
    try:
        while rclpy.ok():
