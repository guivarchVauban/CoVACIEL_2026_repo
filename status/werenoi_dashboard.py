...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header, .grid, footer, #music-bar { position: relative; z-index: 2; }
  ...
  header h1 {
    ...
    letter-spacing: 4px;
    color: var(--accent);
    text-shadow: 0 0 30px rgba(200,120,255,0.8), 0 0 60px rgba(200,120,255,0.3);
  }
  header h1 span { color: var(--green); text-shadow: 0 0 20px rgba(122,255,176,0.7); }
  #conn-dot {
  ...
  #music-bar {
    ...
    background: rgba(120,50,200,0.25);
  ...
  #music-bar span { font-size: 11px; color: var(--accent); letter-spacing: 2px; }
  #music-toggle {
    background: var(--purple); border: none; border-radius: 50%;
    width: 28px; height: 28px; cursor: pointer; font-size: 12px;
    display: flex; align-items: center; justify-content: center;
    transition: background .2s; box-shadow: 0 0 10px rgba(168,85,247,0.6);
  }
  #music-toggle:hover { background: var(--accent); }
  #music-viz {
  ...
  .viz-bar:nth-child(3) { animation-delay: 0.2s;  height: 6px; }
  .viz-bar:nth-child(4) { animation-delay: 0.05s; height: 18px; }
  .viz-bar:nth-child(5) { animation-delay: 0.15s; height: 10px; }
  @keyframes vizanim { from { transform: scaleY(0.3); } to { transform: scaleY(1); } }
  .viz-bar.paused { animation-play-state: paused; }
  ...
  .panel-title {
    font-family: var(--title); font-size: 8px; letter-spacing: 3px;
    color: var(--dim); text-transform: uppercase; margin-bottom: 10px;
  }

  .mode-badge {
    font-family: var(--title); font-size: 32px; font-weight: 900;
    letter-spacing: 2px; text-align: center; padding: 10px 0;
    transition: color .3s, text-shadow .3s;
  }
  .mode-badge.m0 { color: rgba(200,160,255,0.3); }
  .mode-badge.m1 { color: var(--green);  text-shadow: 0 0 24px var(--green); }
  .mode-badge.m2 { color: var(--orange); text-shadow: 0 0 24px var(--orange); }
  .mode-badge.m3 { color: var(--accent); text-shadow: 0 0 24px var(--accent); }
  ...
  .pill {
    ...
    font-size: 11px; letter-spacing: 1px; transition: all .3s;
  }
  .pill.ok   { border-color: var(--green);  color: var(--green);  background: rgba(122,255,176,.1); }
  .pill.warn { border-color: var(--red);    color: var(--red);    background: rgba(255,68,102,.12); box-shadow: 0 0 10px rgba(255,68,102,.3); }
  .pill.off  { border-color: rgba(200,160,255,.2); color: var(--dim); background: transparent; }

  .row {
    display: flex; justify-content: space-between; align-items: baseline;
    padding: 5px 0; border-bottom: 1px solid rgba(180,100,255,.1);
  }
  .row:last-child { border-bottom: none; }
  .row .label { color: var(--dim); font-size: 11px; }
  .row .val   { font-size: 16px; color: var(--text); transition: color .2s; }
  .row .val.hi { color: var(--yellow); text-shadow: 0 0 8px rgba(255,229,102,.5); }
  .row .unit  { font-size: 10px; color: var(--dim); margin-left: 3px; }

  .bar-wrap { height: 6px; background: rgba(255,255,255,.06); border-radius: 10px; margin-top: 6px; overflow: hidden; }
  .bar-inner { height: 100%; border-radius: 10px; transition: width .3s, background .3s; }

  #servo-arc { display: block; margin: 8px auto 0; }

  /* TF */
  .tf-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 6px; margin-top: 4px; }
  .tf-item {
    display: flex; align-items: center; justify-content: space-between;
    padding: 5px 8px; border-radius: 8px; border: 1px solid rgba(180,100,255,.15);
    background: rgba(80,20,120,.2); transition: border-color .3s;
  }
  .tf-item.fresh  { border-color: rgba(122,255,176,.3); }
  .tf-item.stale  { border-color: rgba(255,68,102,.4); background: rgba(255,68,102,.05); }
  .tf-item.static { border-color: rgba(168,85,247,.35); }
  .tf-name { font-size: 10px; color: var(--text); }
  .tf-age.fresh  { font-size: 10px; color: var(--green); }
  .tf-age.stale  { font-size: 10px; color: var(--red); }
  .tf-age.static { font-size: 10px; color: var(--purple); }

  /* Health */
  .health-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 8px; margin-top: 4px; }
  .health-card {
    padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(180,100,255,.15);
    background: rgba(50,10,80,.3); transition: all .3s;
  }
  .health-card.ok   { border-color: rgba(122,255,176,.35); background: rgba(122,255,176,.05); }
  .health-card.dead { border-color: rgba(255,68,102,.4); background: rgba(255,68,102,.07); animation: deadblink 1s ease-in-out infinite; }
  @keyframes deadblink { 0%,100%{opacity:1} 50%{opacity:.55} }
  .health-topic { font-size: 10px; color: var(--dim); margin-bottom: 4px; }
  .health-hz { font-family: var(--title); font-size: 22px; font-weight: 700; }
  .health-hz.ok   { color: var(--green); text-shadow: 0 0 10px rgba(122,255,176,.4); }
  .health-hz.dead { color: var(--red); }
  .health-unit { font-size: 10px; color: var(--dim); margin-left: 3px; }
  .health-dot { display: inline-block; width: 6px; height: 6px; border-radius: 50%; margin-right: 6px; vertical-align: middle; }
  ...
  .log-entry {
    display: flex; gap: 8px; padding: 5px 10px; border-radius: 6px;
    border-left: 2px solid rgba(200,160,255,.2); background: rgba(50,10,80,.3);
    font-size: 11px; align-items: flex-start;
  }
  .log-entry.WARN  { border-left-color: var(--orange); background: rgba(255,170,68,.07); }
  .log-entry.ERROR { border-left-color: var(--red);    background: rgba(255,68,102,.09); }
  .log-entry.FATAL { border-left-color: var(--red);    background: rgba(255,68,102,.16); animation: deadblink .5s ease-in-out infinite; }
  ...
  /* Nodes */
  .nodes-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(155px, 1fr)); gap: 6px; margin-top: 4px; }
  .node-card {
    display: flex; align-items: center; gap: 8px; padding: 7px 10px;
    border-radius: 8px; border: 1px solid rgba(180,100,255,.15);
    background: rgba(50,10,80,.3); transition: all .3s;
  }
  .node-card.up   { border-color: rgba(122,255,176,.3); background: rgba(122,255,176,.05); }
  .node-card.down { border-color: rgba(255,68,102,.4); background: rgba(255,68,102,.07); animation: deadblink 1.2s ease-in-out infinite; }
  .node-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
  .node-dot.up   { background: var(--green); box-shadow: 0 0 8px var(--green); }
  ...
  .pose-stat .big { font-family: var(--title); font-size: 18px; font-weight: 700; color: var(--yellow); text-shadow: 0 0 10px rgba(255,229,102,.4); }
  .pose-stat .lbl { font-size: 10px; color: var(--dim); margin-top: 2px; letter-spacing: 1px; }

  footer { text-align: right; color: var(--dim); font-size: 10px; margin-top: 14px; letter-spacing: 1px; }
</style>
</head>
<body>
...
<div id="music-bar">
  ...
  <span>WERENOI — PÉTUNIAS</span>
  <audio id="player" src="/music" loop></audio>
</div>
...
const MODE_SUBS   = {0:"En attente", 1:"Wall-following PID", 2:"Man\\u0153uvre demi-tour", 3:"Navigation SLAM"};
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/data":
            ...
            try:
                with open("/root/werenoi.jpg", "rb") as f:
                    img = f.read()
                self.send_response(200)
            ...
            try:
                with open("/root/werenoi.mp3", "rb") as f:
                    mp3 = f.read()
                self.send_response(200)
